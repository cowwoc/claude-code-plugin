...
@${CLAUDE_PLUGIN_ROOT}/.claude/cat/references/principles.md
@${CLAUDE_PLUGIN_ROOT}/.claude/cat/references/questioning.md
@${CLAUDE_PLUGIN_ROOT}/.claude/cat/templates/project.md
@${CLAUDE_PLUGIN_ROOT}/.claude/cat/templates/config.json
...
1. **Abort if project exists:**
   ```bash
   [ -f .planning/PROJECT.md ] && echo "ERROR: Project already initialized. Use /cat:progress" && exit 1
   ```
...
- options:
  - "Map codebase first" — Run /cat:map-codebase to understand existing architecture (Recommended)
  - "Skip mapping" — Proceed with project initialization

**If "Map codebase first":**
```
Run `/cat:map-codebase` first, then return to `/cat:new-project`
```
Exit command.
...
Present completion with next steps (see ${CLAUDE_PLUGIN_ROOT}/.claude/cat/references/continuation-format.md):
